...
int main() {
  ...
  while (1) {
    std::cout << "Enter "
                 "func(forward,back,throttle,left,right,tright,tleft,flm,"
                 "neutral,(arm << danger)): ";
    std::string ch_val;
    std::getline(std::cin, ch_val);
    if (ch_val == "forward" || ch_val == "back" || ch_val == "throttle" ||
        ch_val == "left" || ch_val == "right" || ch_val == "tright" ||
        ch_val == "tleft" || ch_val == "flm" || ch_val == "neutral" ||
        ch_val == "arm") {
      std::string pwm_val;
