...
void *gpsDataStream(void *args) {
  ...
  while (!loop) {
    ...
    while (serialDataAvail(fd)) {
      ...
      string gpsDataString = to_string(round(NazaDecoder.getNumSat())) + "," +
                             to_string(NazaDecoder.getLat()) + "," +
                             to_string(NazaDecoder.getLon()) + "," +
                             to_string(round(NazaDecoder.getHeadingNc())) +
                             "," + to_string(NazaDecoder.getGpsAlt()) + " \n";
      char buffer[1024];
