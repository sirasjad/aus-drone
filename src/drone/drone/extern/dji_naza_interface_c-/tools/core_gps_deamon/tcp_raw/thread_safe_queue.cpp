...
** When dequeing a batch of messages, the return code is the total number of
*bytes
...
public:
  ThreadSafeQueue()
      : _queue(), _queueMutex(), _msgNotification(), _numEnqueued(0) {}
  ...
  size_t size() const { return _numEnqueued; }
...
protected:
  ...
  virtual size_t sizeOfData(const T &) const { return sizeof(T); }
