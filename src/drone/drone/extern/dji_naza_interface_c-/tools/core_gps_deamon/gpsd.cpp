...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  cout << "WebSocket Broadcast Server running on port " << web_socketPort
       << endl;
  /*
  Websockets
  */
  ...
  */
  ...
  while (1) {
    ...
    while (serialDataAvail(fd)) {
      ...
      gpsData = std::to_string(round(NazaDecoder.getNumSat())) + "," +
                std::to_string(NazaDecoder.getLat()) + "," +
                std::to_string(NazaDecoder.getLon()) + "," +
                std::to_string(round(NazaDecoder.getHeadingNc())) + "," +
                std::to_string(NazaDecoder.getGpsAlt()) + " \n";
      strncpy(buffer, gpsData.c_str(), sizeof(buffer));
      // sending GPS data to all clients
      if (clock() % 100 == 0) {
        // std::cout << "GPS Sats: " << round(NazaDecoder.getNumSat()) << ",
        // Lat: " << NazaDecoder.getLat() << ", Long: " << NazaDecoder.getLon()
        // << ", Heading: " << round(NazaDecoder.getHeadingNc()) << ", Alt: " <<
