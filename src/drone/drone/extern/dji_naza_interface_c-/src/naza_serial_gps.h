/*
  ...
  The RC PWM input code taken from
  https://www.instructables.com/id/RC-Quadrotor-Helicopter/step12/Arduino-Demo-PWM-Input/
*/
...
public:
  ...
  void getAll(double &lat, double &lon, double &alt, double &speed,
              double &heading, uint8_t &sats);
  ...
  uint8_t getMonth();
  uint8_t getDay();
  uint8_t getHour(); // Note that for time between 16:00 and 23:59 the hour
                     // returned from GPS module is actually 00:00 - 7:59.
  uint8_t getMinute();
  uint8_t getSecond();
...
private:
  ...
  double
      gpsVsi; // vertical speed indicator (from GPS) in m/s (a.k.a. climb speed)
